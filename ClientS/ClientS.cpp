...
)
{
	...
	all.sin_family = hst->h_addrtype;
	all.sin_addr.s_addr = inet_addr(inet_ntoa(*(in_addr*)hst->h_addr_list[0]));
...
int main()
{
	try {
		...
		GetServerByName(name, call, (sockaddr*)&from, &sizefrom);
